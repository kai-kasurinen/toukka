#
# pylint: disable=wrong-import-position,invalid-name,missing-docstring
...
class PlayerCtlManager:
    ...
    def on_manager_name_appeared(self, manager, name):
        logger.debug('on name appeared: %s, %s', name, name.name)
        self.init_player(name)

    def on_manager_name_vanished(self, manager, name):
        logger.debug('on name vanished: %s, %s', name, name.name)

    def on_manager_player_appeared(self, manager, player):
        logger.debug('on player appeared: %s, %s', player, player.props.player_name)
        self.player_status(player)

    def on_manager_player_vanished(self, manager, player):
        logger.debug('on player vanished: %s, %s', player, player.props.player_name)
    ...
    def init_player(self, name):
        ...
        # FIXME: does player ever removed?
        self.player_manager.manage_player(player)

    def on_player_exit(self, player):
        logger.debug('on exit: %s', player)

    def on_player_loop_status(self, player, loop_status):
        logger.debug('on loop status: %s', loop_status)

    def on_player_seeked(self, player, position):
        logger.debug('on seeked: %s', position)

    def on_player_shuffle(self, player, shuffle_status):
        logger.debug('on shuffle: %s', shuffle_status)

    def on_player_volume(self, player, volume):
        logger.debug('on volume: %s', volume)

    def on_player_metadata(self, player, metadata):
        logger.debug('on metadata: %s', metadata)

    def on_player_playback_status(self, player, status):
        logger.debug('on status: %s', status)

    def player_status(self, player):
        logger.debug('status: player %s status: %s', player.props.player_name, player.props.status)
        logger.debug('status: player %s metadata: %s', player.props.player_name, player.props.metadata)
        logger.debug('status: player %s position: %s', player.props.player_name, player.props.position)
